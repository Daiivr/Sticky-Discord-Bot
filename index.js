...
async function fetchStickyMessage(channelID, channel) {
    try {
        ...
        if (rows.length > 0) {
            const { message_content, embed_color = '#0099ff', embed_image, title } = rows[0];

            // Ensure the title is a non-empty string, else set it to a default value
            const embedTitle = (typeof title === 'string' && title.trim().length > 0) ? title : 'Professor Turo';
